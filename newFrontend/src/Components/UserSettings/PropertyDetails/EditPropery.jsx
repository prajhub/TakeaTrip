...
const EditPropery = ({selectedProperty, onClose}) => {
  ...
  const handleCheckInToChange = (event) => {
    ...
    setValue("checkinTime", checkInTimes)
  };
  ...
  checkOutOptions.push(
    <option key={`12:00`} value={`12:00 PM`}>12:00 PM</option>
  );

  

  const [checkoutTime, setCheckoutTime] = useState(checkOutOptions[0].props.value);
  ...
  const handleAmenityChange = (event) => {
    ...
    if (isChecked) {
      ...
      setValue("amenities", amenities.filter((name) => name !== amenityName));
      return;
    }
  ...
  return (
    <>
        ...
        <input hidden {...register("checkinTime")}/>
                        </div>
                      </div>
                    </div>
                    </section>
