...
const Body = () => {
  ...
  const {data, isFetching} = useGetRoomByIdQuery(id, {
  ...
  return (
   <>
    <section>
        ...
        <div className='max-w-[1000px]  justify-between mx-auto items-center flex flex-wrap py-6 md:mt-6'>
            ...
            <div className='flex flex-col gap-8 mt-5 border-b '>
                ...
                <div className='flex flex-col'>
                    ...
                    <p className='text-md'>{startMonthDay} - {endMonthDay}</p>
                </div>
      ...
      <h2 class="text-black text-lg font-medium  mb-3">Hanuman | A Luxury Pool Villa | 3-Bedroom</h2>
        <div className='border-b'/>
      <p class="text-black mb-2 text-xl mt-6 font-bold">Price Details</p>
      <div className='flex flex-row mb-4 justify-between'>
        <span>$358.61 x {numberOfDays} days</span>
        <span>$1,793.05</span>
      </div>
      <div className='border-b'/>
      <div className='flex flex-row mb-4 mt-4 justify-between'>
        ...
        <div className='max-w-[1000px] mx-auto'>
          <label className='mb-4 text-md font-semibold'>Select room number</label>
          {initialRoomNumbers.map(item => (
  <div className='mt-4'>
    <button 
      key={item.number} 
      className={`py-2.5 px-5 mr-2 mb-2 text-sm font-medium focus:outline-none 
        ${item.userId ? 'bg-gray-400 text-gray-600 cursor-not-allowed' 
    ...
    >
      ...
      >
         <button disabled={!isRoomSelected}  class={`py-2.5 px-5 mr-2 mb-2 mt-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 ${!isRoomSelected && 'opacity-50 cursor-not-allowed'}`}>Confirm & Pay</button>
      </StripeCheckout>
    </section>
   </>
  )
